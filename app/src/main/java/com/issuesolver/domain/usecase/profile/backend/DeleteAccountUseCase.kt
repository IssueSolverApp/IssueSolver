...
class DeleteAccountUseCase @Inject constructor(private val deleteAccountRepository: DeleteAccountRepositoryInterface) {

    suspend operator fun invoke(password: DeleteAccountRequest) = flow {

        emit(Resource.Loading())
        try {
            val response = deleteAccountRepository.deleteAccount(password)
